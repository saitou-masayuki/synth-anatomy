...
- ビブラート・リードのmod1.amt(目標0.04)は既定tol(0.05)だとamt=0でも数式上は許容範囲内になるが、mod1.src/dstが未配線（'none'）である限りenum不一致でステップ自体が達成扱いにならないため実害はない
...
`content-recipes.js`＋`recipe-engine.js`: レシピ10本（プラック/スーパーソー/ウォブル/パッド/ベル/アシッド/サブベース/ビブラートリード/WTモーションパッド/ノイズライザー）。目標音は同エンジンのパッチとして保持（音源ファイル不要）。「initに全ステップ適用≡target」の恒等性テストで数値の矛盾を機械検出。

### Phase 3: 耳トレクイズ

`quiz-engine.js`（純粋ロジック・rng注入）: ベースパッチからランダムに1パラメーター変更→A/B聴き比べ→「どれが変わった？」。Lv1（2択・激変）〜Lv5（6択・微変化・方向当て）。chord-labの弱点重み付き抽選を移植。可聴性はレベルごとの人手設計ベースパッチ＋整合性テストで保証。

### Phase 4: Serum/Vital実機対応表

`content-map.js`: paramIdごとに Serum / Serum 2 / Vital での場所・名称・注記（`match: exact|similar|concept|none` の4値）。ノブ長押しポップアップで表示。網羅性テスト付き。

---

## 検証方法

- **自動テスト**: `node --test test/`（chord-lab方式）。パラメーター定義の範囲・カーブ往復・describe網羅、wavetableスペクトル整合性、LFO/ADSR数式、modスロット解決、（Phase 2+）レシピ恒等性・クイズ出題決定性・対応表網羅性
